...
int *add_vectors(int *v1, int n1, int *v2, int n2) {
  int *result;
  if(n1 != n2) {
    //delete[] result;	//Memory has not been allocated to result yet
    return NULL;
  }
  result = new int[n1];
  for(int i = 0; i < n1; i++)
...
int* divide_vectors(int *v1, int *v2, int n) {
  if( n > sizeof(v1)/sizeof(int) )  //making n equal to size if n is greater than the size of either of the arrays, otherwise
    n = sizeof(v1)/sizeof(int);     //illegal memory accessing, i.e. memory out of bounds
  else if( n > sizeof(v2)/sizeof(int) )
    n = sizeof(v2)/sizeof(int);
  else if( sizeof(v1)/sizeof(int) > sizeof(v2)/sizeof(int) )
    n = sizeof(v2)/sizeof(int);
  else if( sizeof(v1)/sizeof(int) < sizeof(v2)/sizeof(int) )
...
int sumOfSquares(int *v1, int *v2, int n) {
  ...
  for(int i = 0; i < n; i++)
    sum += temp[i];
  delete[] temp;          //delete temp after calculation of result
  return sum;
}
...
int add(int *n1, int *n2) {
  int sum = *n1 + *n2;
  //delete n1;	//Memory has not been allocated with new so
  //delete n2;	//cannot delete them.
  return sum;
}
...
int* sum_vectors(int *v1, int* v2, int nelements) {
  ...
  for(int i = 0; i < nelements; i++)
     vsum[i] = v1[i] + v2[i];
   /*delete[] vsum;*/	//Cannot deallocate because we need to 
   return vsum;		//send vsum back to main. If we did, we would be 
...
void create_and_free(int rows, int cols) {
  ...
  for(int i = 0; i < rows; i++)
    delete[] arr[i];
  delete [] arr;	//Deallocate memory here, otherwise, premature deallocation.
}
...
void reverse(char *arr, int len) {
  ...
  if(len == 0 || len == 1) {//no need to reverse
    delete[] arr2;
    return ;  //return, otherwise double deallocation.
  }
  else {
    for(int i = 0 ; i < len; i++)
      arr2[i] = arr[i];
